...
export default function RecordList() {
    ...
    return (
        <div className="mx-auto w-full min-h-[100svh] md:min-h-[100%] bg-background">
            {/* 헤더 */}
            <header className="w-full pt-10 pb-5 bg-white rounded-b-[50px]">
                <Title>녹음 기록</Title>
            </header>
                ...
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`h-8 rounded-full px-4 text-sm font-medium transition ${tab === "recent"
                        ...
                        <button
                            onClick={() => { setTab("date"); setShowCal(true); }}
                            className={`h-8 rounded-full px-4 text-sm font-medium transition ${tab === "date" ? "bg-button-nav text-white shadow"
                                    : "bg-white text-slate-600 border border-slate-200"
                                }`}
                        >
                            날짜
                        ...
                        >
                            <div className="flex w-full items-center gap-3 text-left">
                                ...
                                <div className="mb-1 flex items-center justify-between">
                                    ...
                                    <span className="text-xs text-slate-500">{rec.dateLabel}</span>
                                </div>
                                <p className="line-clamp-1 text-sm text-slate-500">{rec.desc}</p>
                                </div>

                                <div className="relative w-10 h-10">
                                    <div className={`absolute inset-0 transition-opacity duration-200 ${isEditing ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
                                        <button
                                    ...
                                    <div className={`absolute inset-0 transition-opacity duration-200 ${isEditing ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                                        <button
            ...
            <CalendarBottomSheet
                ...
                setSelected={(d) => { setSelectedDate(d); setShowCal(false); }}
            />
